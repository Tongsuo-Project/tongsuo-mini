...
static int sm4_set_key(const uint8_t *key, uint32_t rk[SM4_KEY_SCHEDULE])
{
    /*
     * Family Key
     */
     ...
     * Constant Key
     */
    static const uint32_t CK[32] = {0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269, 0x70777E85,
...
static void sm4_decrypt(const uint8_t *in, uint8_t *out, uint32_t rk[SM4_KEY_SCHEDULE])
{
    uint32_t B0 = load_u32_be(in, 0);
    uint32_t B1 = load_u32_be(in, 1);
    uint32_t B2 = load_u32_be(in, 2);
...
static void cbc128_decrypt(const unsigned char *in, unsigned char *out, size_t len, const void *key,
                           unsigned char ivec[16], block128_f block)
{
    size_t n;
    union {
...
static int sm4_cbc_cipher(TSM_SM4_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len)
{
    if (ctx->flags & TSM_CIPH_FLAG_ENCRYPT) {
        cbc128_encrypt(in, out, len, ctx->rk, ctx->iv, (block128_f)sm4_encrypt);
    } else {
        cbc128_decrypt(in, out, len, ctx->rk, ctx->iv, (block128_f)sm4_decrypt);
    }
                                ...
                                unsigned char *out, size_t *outl)
{
    ...
    if (ctx->mode == TSM_CIPH_MODE_CBC) {
        cipher = sm4_cbc_cipher;
    } else {
        LOGE(tsm_err2str(TSM_ERR_WRONG_CIPH_MODE));
        return TSM_ERR_WRONG_CIPH_MODE;
    }
    ...
    if (ctx->buf_len == 0 && (inl & (bl - 1)) == 0) {
        if (cipher(ctx, out, in, inl) == TSM_OK) {
            *outl = inl;
            return TSM_OK;
        } else {
            *outl = 0;
        ...
        if (bl - i > inl) {
            ...
            *outl = 0;
            ...
            if (((inl - j) & ~(bl - 1)) > INT_MAX - bl) {
                LOGE(tsm_err2str(TSM_ERR_BUFFER_OVERFLOW));
                return TSM_ERR_BUFFER_OVERFLOW;
            }
            memcpy(&(ctx->buf[i]), in, j);
            inl -= j;
            in += j;
            if (cipher(ctx, out, ctx->buf, bl) != TSM_OK)
                return TSM_FAILED;
            out += bl;
            *outl = bl;
        }
                                  ...
                                  unsigned char *out, size_t *outl)
{
    ...
    if (ctx->flags & TSM_CIPH_FLAG_NO_PAD)
        return tsm_sm4_crypt_update(ctx, in, inl, out, outl);
    ...
    if (tsm_sm4_crypt_update(ctx, in, inl, out, outl) != TSM_OK)
        return TSM_FAILED;
...
int tsm_sm4_update(void *ctx, const unsigned char *in, size_t inl, unsigned char *out, size_t *outl)
{
    ...
    if (c->flags & TSM_CIPH_FLAG_ENCRYPT) {
        ret = tsm_sm4_encrypt_update(ctx, in, inl, out, outl);
    } else {
...
static int tsm_sm4_decrypt_final(TSM_SM4_CTX *ctx, unsigned char *out, size_t *outl)
{
    ...
    *outl = 0;
    ...
    if (ctx->flags & TSM_CIPH_FLAG_NO_PAD) {
        if (ctx->buf_len) {
            LOGE(tsm_err2str(TSM_ERR_DATA_NOT_MULTIPLE_OF_BLOCK_LENGTH));
            return TSM_ERR_DATA_NOT_MULTIPLE_OF_BLOCK_LENGTH;
        }
        *outl = 0;
        return TSM_OK;
    }
    if (b > 1) {
        if (ctx->buf_len || !ctx->final_used) {
            LOGE(tsm_err2str(TSM_ERR_WRONG_FINAL_BLOCK_LENGTH));
            return TSM_ERR_WRONG_FINAL_BLOCK_LENGTH;
        }
        assert(b <= sizeof(ctx->final));
        ...
        n = ctx->final[b - 1];
        if (n == 0 || n > (int)b) {
            LOGE(tsm_err2str(TSM_ERR_BAD_DECRYPT));
            return TSM_ERR_BAD_DECRYPT;
        }
        for (i = 0; i < n; i++) {
            if (ctx->final[--b] != n) {
                LOGE(tsm_err2str(TSM_ERR_BAD_DECRYPT));
...
int tsm_sm4_final(void *ctx, unsigned char *out, size_t *outl)
{
    ...
    if (c->flags & TSM_CIPH_FLAG_ENCRYPT)
        ret = tsm_sm4_encrypt_final(ctx, out, outl);
    else
